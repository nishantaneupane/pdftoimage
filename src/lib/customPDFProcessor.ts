...
export class CustomPDFProcessor {
  ...
  private parseBasicPDFInfo(): { pageCount: number } {
    ...
    if (pageMatches && pageMatches.length > 0) {
      ...
      pageCount = Math.max(...pageMatches.map(match => {
  ...
  private createPageCanvas(pageNumber: number, width: number, height: number): HTMLCanvasElement {
  ...
  private renderTextContent(ctx: CanvasRenderingContext2D, text: string, width: number, height: number, pageNumber: number): void {
    ctx.fillStyle = 'black';
    ...
    lines.forEach((line, index) => {
      if (y < height - 40 && index < 20) { // Limit to prevent overflow
  ...
  private renderPlaceholder(ctx: CanvasRenderingContext2D, width: number, height: number, pageNumber: number): void {
    ctx.fillStyle = '#666';
    ...
    ctx.fillStyle = '#999';
    ctx.fillText('(Content extracted and rendered)', width / 2, height / 2 + 20);
  }
  ...
  private extractBasicTextForPage(pageNumber: number): string {
    ...
    const wordsPerPage = Math.max(50, Math.floor(words.length / this.parseBasicPDFInfo().pageCount));
